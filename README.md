# A* 路径规划器（支持任意形状机器人 + 扫掠碰撞 + 走过的栅格可视化）

一个基于纯 A* 的移动机器人路径规划示例，支持任意形状（通过 ASCII 图案定义）的机器人，在 `(x, y, yaw)` 三维状态空间中进行搜索，并采用连续扫掠碰撞检测保证运动安全。动画运行时会实时标记机器人走过的栅格，方便直观查看探索/行驶轨迹。
...
- **任意形状机器人**  
  通过 ASCII 图案定义机器人 footprint（内置 `T` / `A` / `F` 三种示例），可自由扩展。
...
- **走过的栅格可视化**  
  动画运行时持续把机器人已经过的栅格标记为不同颜色，支持两种模式：
  - `center`：仅标记机器人中心经过的栅格；
  - `footprint`：标记机器人 footprint 覆盖过的所有栅格。

- **一键生成 GIF**  
  自动保存动画为 GIF，方便放入文档、README 或演示 PPT。
